...
void loop()
{
  digitalWrite(8, HIGH);// Heartbeat Signal to Main Nano (int pin 3) change 12-04-2016
  ack = digitalRead (12);
  if (ack == 1)
     {
  // Send Acknowledge Beeb to TX Radio
    digitalWrite(4, HIGH);// Key the radio pin 4
    delay(2500); // Wait for Activated PTT on TX Radio
    // play a dash
  ...
  delay(inter);
    ...
    digitalWrite(8, LOW);// Heartbeat Signal to Main Nano (int pin 3) change 12-04-2016
     }
  ...
  if ((int32_t) (millis() - next_aprs) >= 0) {
    ...
    delay(3500); // Wait for Activated PTT on TX Radio and CS Release from Main Arduino
