// Projet Otachi-Arduino Sonde Stratosphérique par Nanikana-Aerospace.
// Ballon stratosphérique avec Arduino Uno R3 et PixHawk + GY80 + Ublox 6 + Relay
// Code basé sur des sketchs sous licence Open Source
// Assemblage et programmation par Josianne Gilbert ( Nanikana-Aerospace )
// www.nanikana-aerospace.org  2014-2015
// Version 20oct2014
...
void loop(){
  ...
  Serial.print("PTT TX   : ON");Serial.println();
    Serial.print("LAT      : "); Serial.println(buffer1_lat);
    Serial.print("LON      : ");Serial.println(buffer0_long);
  Serial.print("CW ID    : NANIKANA-AEROSPACE");Serial.println();
  ...
  delay(2000);
   morseWord = encode(buffer0_long);
      for(int i=0; i<=morseWord.length(); i++){
       ...
       // Wait 
  delay(2000);
   morseWord = encode("NANIKANA:AEROSPACE");
  ...
  digitalWrite(RELAY1,HIGH);          // Turns Relay PTT Off
   ...
   Serial.print("WAIT     : 2 min. before next CW TX");Serial.println();
   float temperature = bmp085GetTemperature(bmp085ReadUT()); //MUST be called first
  float pressure = bmp085GetPressure(bmp085ReadUP());
  //float tempC = sensors.getTempC(deviceAddress);//Jojo
...
Serial.println();//line break
  ...
  Serial.println(" M");
  Serial.print("CPU Batt.: ");
  Serial.print( readVcc(), DEC );
  Serial.println(" mV");
  Serial.println();//line break

  delay(120000); //wait 2 minutes and get values again.
   }
  }
