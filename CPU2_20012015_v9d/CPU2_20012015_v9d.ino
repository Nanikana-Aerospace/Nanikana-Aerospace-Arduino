...
***  Otachi Flight COMP Prog. v.2001205-D  ***
...
void setup()
{
  ...
  ss.begin(GPSBaud);
  Serial.println("");
  Serial.println("Otachi Flight COMP Prog. v.20012015 (v.9d)");
...
void loop()
{
  ...
  if (rxstr == 53) {Serial.println ("MCODE RECEIVE"); mcode = 1; digitalWrite(4, HIGH); delay (250); digitalWrite(4, LOW);}
  if (rxstr == 48 && mcode == 1) {Serial.println ("STOP TX CMD RECEIVE"); mcode = 0; txstate = 0; digitalWrite(4, HIGH); delay (500); digitalWrite(4, LOW);}
  if (rxstr == 49 && mcode == 1) {Serial.println ("START TX CMD RECEIVE"); mcode = 0; txstate = 1; digitalWrite(4, HIGH);delay (500); digitalWrite(4, LOW);}
  ...
  Serial.print (rxstr, DEC); // Command Receive from RX Arduino (DTMF)
  Serial.print("!"); //BEGIN OF DATA TO TX in APRS
