...
Sonde stratosphérique avec Arduino Nano  + Ublox 6 + Gy80 +ML8511 +Solar Pannel +DS18B20
Include command for RadioShield from ArgentData System
...
***  Otachi Flight COMP Prog. v.04122015-H  ***
 ...
 D6 To (CS) RX Radio
 D11 to Send Acknowledgment Beep
...
void setup()
{
  ...
  ss.begin(GPSBaud);
  Serial.println("");
  Serial.println("Otachi Flight COMP Prog. v.04122015-H (v9)");
  ...
  pinMode (11, OUTPUT); // Acknowledgment Beep
  pinMode(UVOUT, INPUT);
...
void loop()
{
  ...
  if (rxstr == "6615") {Serial.println ("MCODE RECEIVE"); mcode = 1; }
  if (rxstr == "5335" && mcode == 1) {Serial.println ("STOP TX CMD RECEIVE"); mcode = 0; }
  if (rxstr == "5111" && mcode == 1) {Serial.println ("START TX CMD RECEIVE"); mcode = 0; }
  ...
  Serial.print (rxstr); // Command Receive from RX Arduino (DTMF)
  Serial.print("!"); //BEGIN OF DATA TO TX in APRS
...
// You must first get a temperature measurement to perform a pressure reading.
  ...
  if (status != 0)
  {
    ...
    if (status != 0)
    {
      ...
      if (status != 0)
      {
        ...
        else Serial.print("error pressure");
      }
      else Serial.print("error starting pressure");
    }
    else Serial.print("error temperature");
  }
  else Serial.print("error starting temperature");
  ...
  Serial.print (heartbeatrx); Serial.print(" "); Serial.print (heartbeattx);
  
  if (heartbeattx == 1) digitalWrite(6, HIGH); else digitalWrite(6, LOW);// If heartbeat/tx = 1 then CS Signal to Radio else stay LOW  
  Serial.print("\r\n");
